...
export default async function ReviewPage({ params }: Props) {
  ...
  const [{ data: startup }, { data: member }] = await Promise.all([
    adminClient.from('startups').select('*').eq('id', startupId).single<Startup>(),
    adminClient.from('ic_members').select('*').eq('email', user.email!).maybeSingle<IcMember>(),
  ])

  if (!startup) notFound()
  if (!member) redirect('/review')

  // Check access: PreScreen sees all, All sees all, explicit assignment, or matching sector
  const isPreScreen = member.ic_type === 'PreScreen'
  const hasAccess =
    isPreScreen ||
    member.ic_type === 'All' ||
    member.ic_type === startup.sector

  // Also check explicit assignment
  if (!hasAccess) {
    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-brand-dark shadow-md">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <span className="text-white/40 text-sm hidden sm:block ml-auto">{member.name}</span>
        </div>
      </nav>
